...
export default function EducationalBackgroundSection({
  ...
  useEffect(() => setAdditionalQuals(initialAdditional || []), [initialAdditional]);
   ...
   // Form for initial setup (school, year, index, combination)
  ...
  const handleSaveSetup = async () => {
    setIsSaving(true);
    try {
      await AxiosInstance.post(`/api/admissions/update_education_setup/${application?.id}/`, {
        ...
        <Table size="small">
          <TableBody>
            {results.length > 0 ? (
              results.map((r, i) => (
                <TableRow key={i}>
                  <TableCell><Typography variant="body2" fontWeight={500}>{r.subject?.name}</Typography></TableCell>
                  <TableCell align="right">{r.grade && <Chip label={r.grade} size="small" color="primary" />}</TableCell>
                </TableRow>
              ))
            ) : (
              <TableRow>
                <TableCell colSpan={2} align="center">
                  <Typography color="text.secondary" py={3}>No results added yet</Typography>
                </TableCell>
              </TableRow>
            )}
          </TableBody>
  ...
  return (
    <Card sx={{ boxShadow: 1, "&:hover": { boxShadow: 3 } }}>
      ...
      <Dialog open={openSetupModal} onClose={() => setOpenSetupModal(false)} maxWidth="sm" fullWidth>
        <DialogTitle>Education Setup</DialogTitle>
        <DialogContent sx={{ mt: 2 }}>
          <Alert severity="info" sx={{ mb: 3 }}>
            Fill in your examination details to enable result entry.
          </Alert>

          <FormControlLabel
            control={<Checkbox checked={setupForm.has_olevel} onChange={e => setSetupForm(prev => ({ ...prev, has_olevel: e.target.checked }))} />}
            label="I have O-Level (UCE)"
          />

          {setupForm.has_olevel && (
            <Box sx={{ mt: 2, display: "flex", flexDirection: "column", gap: 2 }}>
              <TextField label="O-Level School" fullWidth value={setupForm.olevel_school} onChange={e => setSetupForm(p => ({...p, olevel_school: e.target.value}))} />
              <TextField label="Index Number" fullWidth value={setupForm.olevel_index_number} onChange={e => setSetupForm(p => ({...p, olevel_index_number: e.target.value}))} />
              <TextField label="Year" type="number" fullWidth value={setupForm.olevel_year} onChange={e => setSetupForm(p => ({...p, olevel_year: e.target.value}))} />
            </Box>
          )}
          ...
          <FormControlLabel
            control={<Checkbox checked={setupForm.has_alevel} onChange={e => setSetupForm(prev => ({ ...prev, has_alevel: e.target.checked }))} />}
            label="I have A-Level (UACE)"
          />
          ...
          {setupForm.has_alevel && (
            <Box sx={{ mt: 2, display: "flex", flexDirection: "column", gap: 2 }}>
              ...
              <TextField label="Combination (e.g. PCM, PCB, Arts)" fullWidth value={setupForm.alevel_combination} onChange={e => setSetupForm(p => ({...p, alevel_combination: e.target.value}))} />
            </Box>
          )}
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setOpenSetupModal(false)}>Cancel</Button>
          <Button variant="contained" onClick={handleSaveSetup} disabled={isSaving}>
