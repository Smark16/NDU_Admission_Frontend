...
function normalizeList(raw: unknown): Application[] {
  if (Array.isArray(raw)) return raw as Application[]
  if (raw && typeof raw === "object" && Array.isArray((raw as { results?: unknown }).results)) {
    return (raw as { results: Application[] }).results
  }
  return []
}

export default function RevokedList() {
  ...
  return (
    <Box sx={{ p: 3, background: "linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%)", minHeight: "100vh" }}>
      <Box sx={{ mb: 4, display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 2 }}>
        <Box>
          <Typography variant="h4" sx={{ color: "#0D0060", fontWeight: "bold" }}>Revoked Admissions</Typography>
          <Typography variant="body2" color="text.secondary">
            Students whose admission was withdrawn — review and restore from the application file
          </Typography>
        </Box>
        <Button
            ...
            <Table sx={{ minWidth: 750 }}>
              <TableHead sx={{ backgroundColor: "#f5f7fa" }}>
                <TableRow>
                  ...
                  <TableCell sx={{ fontWeight: "bold" }}>Revoked by</TableCell>
                ...
                {paginatedApplications.length > 0 ? (
                  paginatedApplications.map((app, idx) => (
                    <TableRow key={app.id} hover selected={selected.includes(app.id)} sx={{ "&:hover": { backgroundColor: "#fafafa" } }}>
                      ...
                      <TableCell sx={{ maxWidth: 280 }}>
                        {app.revocation_reason || app.review_notes ? (
                          <Tooltip title={app.revocation_reason || app.review_notes || ""}>
                            <Typography variant="body2" noWrap>{app.revocation_reason || app.review_notes}</Typography>
                          </Tooltip>
                        ) : (
                  ...
                  <TableRow>
                    <TableCell colSpan={10} align="center" sx={{ py: 5 }}>
                      <Alert severity="info">No revoked admissions found.</Alert>
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
