...
interface Application {
  ...
  last_name: string
  gender: string
  status: "submitted" | "accepted" | "rejected" | "under_review" | "admitted"
  created_at: string
  email: string
...
export default function ApplicationList() {
  ...
  const filteredApplications = useMemo(() => {
    return applications.filter((app) => {
      ...
      const matchesFee =
        feeFilter === "all" ||
        (feeFilter === "paid" && app.application_fee_paid) ||
        (feeFilter === "unpaid" && !app.application_fee_paid)
    ...
    <Box sx={{ p: 3, background: "linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%)", minHeight: "100vh" }}>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {[
          ...
          { label: "Fee Paid", value: applications.filter((a) => a.application_fee_paid).length },
          { label: "Fee Pending", value: applications.filter((a) => !a.application_fee_paid).length },
      ...
      <Paper sx={{ p: 3, mb: 3, borderRadius: 2, boxShadow: 3 }}>
        <Grid container spacing={2}>
          ...
          <Grid size={{ xs: 12, sm: 3 }}>
            <FormControl fullWidth size="small">
              <InputLabel>Fee Payment</InputLabel>
              <Select
                value={feeFilter}
                label="Fee Payment"
                onChange={(e) => { setFeeFilter(e.target.value); setPage(0) }}
              >
                <MenuItem value="all">All</MenuItem>
                <MenuItem value="paid">Fee Paid ✓</MenuItem>
                <MenuItem value="unpaid">Fee Not Paid</MenuItem>
              </Select>
            </FormControl>
          </Grid>
        </Grid>
                ...
                {paginatedApplications.length > 0 ? (
                  paginatedApplications.map((app, idx) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <Chip
                          label={app.application_fee_paid ? "Paid" : "Not Paid"}
                          color={app.application_fee_paid ? "success" : "warning"}
                          size="small"
                        />
                      </TableCell>
