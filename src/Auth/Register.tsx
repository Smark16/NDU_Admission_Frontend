      ...
      if (data && typeof data === "object") {
        for (const key of Object.keys(data)) {
          ...
          if (Array.isArray(val)) {
            val.forEach((m: string) => messages.push(key === "non_field_errors" ? m : `${key}: ${m}`))
          } else if (typeof val === "string") {
            messages.push(key === "non_field_errors" ? val : `${key}: ${val}`)
          }
        }
      }
      setRegisterErrors(messages.length ? messages : ["Registration failed. Please try again."])
      setSuccess(false)
    } finally {
      setLoading(false)
    }
